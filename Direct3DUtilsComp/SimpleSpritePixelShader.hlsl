...
{	
	...
	{
		float4 tex =shaderTexture[0].Sample(SampleType, input.tex);
			float3 Rca;
			 float Ra;
			float3 Sca = tex.rgb;
			 float Sa  = tex.a;
			float3 Dca = color.rgb;
			 float Da  = color.a;
					if(blendModeInfo.w == 0)
					{
							...
							//Rca = Da*Dca*Sa + Sca * (1.0 - Sa)*(1.0-Da);
						    Ra  = Sa;
							fillMode = true;
	...
	{	
		...
		if(!fillMode)
		{
			tex =shaderTexture[0].Sample(SampleType, input.tex);
		}else
		{
			tex = textureColor;
		}
		...
		//if(tBlendMode!=0){
			 float3  Sca =tex.rgb;// float3(tex[0],tex[1],tex[3]);
				
			 float Sa  = tex.a;
			 float  msk =0.0;// texture2D(tMask, texCoords).a;
		   //"highp vec2 destCoords=(gl_FragCoord.xy-uDestRect.xy)*uDestRect.zw;"
			 //return float4(input.dtexc.x/input.dtexc.y,0,0,1.0f);
			...
			if (tBlendMode == 2) { // Multiply
				Rca = Sca * Dca + Sca * (1.0 - Da) + Dca * (1.0 - Sa);
				Ra  = Sa + Da - Sa * Da;
			}else

			if (tBlendMode == 3) { // Screen
				Rca = Sca + Dca - Sca * Dca;
				Ra  = Sa + Da - Sa * Da;
			}else

			if (tBlendMode == 4) { // Overlay
			...
			if (tBlendMode == 11) { // Difference
				Rca = Sca + Dca - 2.0 * min(Sca * Da, Dca * Sa);
				Ra  = Sa + Da - Sa * Da;
			}else

			if (tBlendMode == 12) { // Exclusion
				Rca = (Sca * Da + Dca * Sa - 2.0 * Sca * Dca) + Sca * (1.0 - Da) + Dca * (1.0 - Sa);
				Ra  = Sa + Da - Sa * Da;
			}else

			if (tBlendMode == 17) { // Clear
				Rca = float3(0.0f,0.0f,0.0f);
				Ra  = 0.0;
			}else

			if (tBlendMode == 18) { // Copy
				Rca = Sca;
				Ra  = Sa;
			}else

			if (tBlendMode == 19) { // SourceIn
				Rca = Sca * Da;
				Ra  = Sa * Da;
			}else

			if (tBlendMode == 20) { // SourceOut
				Rca = Sca * (1.0 - Da);
				Ra  = Sa * (1.0 - Da);
			}else

			if (tBlendMode == 21) { // SourceAtop
				Rca = Sca * Da + Dca * (1.0 - Sa);
				Ra  = Da;
			}else


			if (tBlendMode == 22) { // DestinationOver
				Rca = Dca + Sca * (1.0 - Da);
				Ra  = Sa + Da - Sa * Da;
			}else

			if (tBlendMode == 23) { // DestinationIn
				Rca = Dca * Sa;
				Ra  = Sa * Da;
			}else

			if (tBlendMode == 24) { // DestinationOut
				Rca = Dca * (1.0 - Sa);
				Ra  = Da * (1.0 - Sa);
			}else

			if (tBlendMode == 25) { // DestinationAtop
				Rca = Dca * Sa + Sca * (1.0 - Da);
				Ra  = Sa;
			}else

			if (tBlendMode == 26) { // XOR
				Rca = Sca * (1.0 - Da) + Dca * (1.0 - Sa);
				Ra  = Sa + Da - 2.0 * Sa * Da;
			}else

			if (tBlendMode == 27) { // PlusDarker
				Rca = Sca + Dca - Sa * Da;
				Ra  = Sa + Da - Sa * Da;
